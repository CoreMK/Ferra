...
const submitLead=form=>form.onsubmit=async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(form));const english=document.documentElement.lang.startsWith('en');['utm_source','utm_medium','utm_campaign'].forEach(key=>data[key]=new URLSearchParams(location.search).get(key)||'');const output=form.querySelector('output');try{const response=await fetch(Ferreta.rest,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});if(!response.ok)throw new Error('lead');form.reset();output.textContent=english?'Thank you. A manager will contact you shortly.':'Дякуємо! Менеджер зв’яжеться з вами найближчим часом.'}catch(error){output.textContent=english?'We could not send your request. Please try again.':'Не вдалося надіслати заявку. Спробуйте ще раз.'}};
