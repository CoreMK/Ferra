...
<header class="site-header">
  ...
  <div class="nav-shell">
    ...
    <div class="nav-actions"><button aria-label="Пошук" data-search-toggle>⌕</button><a href="<?=esc_url(home_url('/favorites/'))?>" aria-label="Обране">♡</a><a href="<?=esc_url(home_url('/compare/'))?>" aria-label="Порівняння">⇄</a><a href="<?=esc_url(wc_get_cart_url())?>" class="cart-link">Кошик <b><?=$cartCount?></b></a><span class="language-switcher"><?=ferreta_language_switcher()?></span></div>
  </div>
  ...
  <form class="search-panel" data-search action="<?=esc_url(home_url('/'))?>"><label>Пошук за назвою або SKU<input name="s" type="search" autocomplete="off" placeholder="Наприклад, ST-75-1"></label><input type="hidden" name="post_type" value="product"><?php if(ferreta_language()==='en'):?><input type="hidden" name="lang" value="en"><?php endif;?><button>Шукати</button></form>
</header>
