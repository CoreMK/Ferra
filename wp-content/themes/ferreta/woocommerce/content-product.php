...
<li <?php wc_product_class('catalog-card',$product); ?>>
  ...
  <div class="catalog-card__body"><small><?=esc_html($product->get_sku())?></small><h2><a href="<?=esc_url(get_permalink($product->get_id()))?>"><?=esc_html($product->get_name())?></a></h2><div class="catalog-card__bottom"><span><?=wp_kses_post($product->get_price_html())?></span><?php if($product->is_purchasable()&&$product->is_in_stock()):?><a class="catalog-card__add add_to_cart_button ajax_add_to_cart" data-product_id="<?=$product->get_id()?>" href="<?=esc_url($product->add_to_cart_url())?>" aria-label="Додати <?=esc_attr($product->get_name())?> до кошика">+</a><?php endif;?></div></div>
</li>
