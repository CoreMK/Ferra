...
<main class="product-page">
  ...
  <?php if($relatedIds):?><section class="related-custom"><div class="section-head"><p class="kicker">ДОПОВНІТЬ КОМПОЗИЦІЮ</p><h2>Схожі <em>предмети.</em></h2><a href="<?=esc_url(wc_get_page_permalink('shop'))?>">Увесь каталог →</a></div><div class="related-grid"><?php foreach($relatedIds as $relatedId):$related=wc_get_product($relatedId);if(!$related)continue;?><article><a href="<?=esc_url(get_permalink($relatedId))?>"><?=wp_get_attachment_image($related->get_image_id(),'medium',false,['loading'=>'lazy'])?></a><div><small><?=esc_html($related->get_sku())?></small><h3><a href="<?=esc_url(get_permalink($relatedId))?>"><?=esc_html($related->get_name())?></a></h3><p><?=wp_kses_post($related->get_price_html())?></p><a class="related-add" href="<?=esc_url($related->add_to_cart_url())?>">До кошика <span>+</span></a></div></article><?php endforeach;?></div></section><?php endif;?>
</main>
